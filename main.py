    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                break
            if response.status_code == 400 or response1.status_code == 500:
                st.subheader('Попробуйте повторить запрос позднее или выбрать другие даты')
                exit()
            if response.status_code == 201 or response1.status_code == 202:
...
if __name__ == '__main__':
    ...
    with tab_single_campaign:
        ...
        #df_wrk_single = campaign_report(start_time = start_point_single, end_time = end_point_single, campaign = campaign)
    ...
    with tab_multiple_campaign:
        ...
        try:
            ...
            #df_multiple_ctr_wrk['CostPerMile'] = df_multiple_ctr_wrk['Cost'] / df_multiple_ctr_wrk['Impressions'] * 1000
            ...
            for i in range(df_multiple_ctr_wrk.index.min(), df_multiple_ctr_wrk.index.max() + 1):
                ...
                #if df_multiple_ctr_wrk.CostPerMile[i] == np.inf or df_multiple_ctr_wrk.CostPerMile[i] == np.nan:
                    #df_multiple_ctr_wrk.CostPerMile[i] = None
            ...
            with st.form("Расчет ДРР"):
                revenue = st.number_input("Введите ваш доход от рекламы")

                submit_drr = st.form_submit_button("Посчитать ДРР")
        ...
        ##рекомендации по конверсиям
            ...
            if response.status_code in [400, 500, 502]:
                ...
                exit()
        st.divider()
        st.subheader("У данных кампаний за выбранный период CR выше медианны")
        ...
        st.divider()
        st.subheader("У данных кампаний за выбранный период времени цена цели выше медианной цены цели, но не в 2 раза. Рассмотрите РК подробнее")
        ...
        st.divider()
        st.subheader("У данных кампаний за выбранный период времени цена цели выше медианной цены цели в 2 раза. Рассмотрите РК подробнее")
    ...
    with tab_key_phrase:
        ...
        try:
            ...
            st.subheader("Ключевые слова за выбранный период, у которых стоимость конверсии выше медианного уровня на 50%")
            ...
            st.subheader("Ключевые слова, по которым расходы составили свыше 150% медианной цены конверсии и которые до сих пор не принесли конверсии")
