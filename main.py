...
if __name__ == '__main__':
    ...
    with tab_multiple_campaign:
        ...
        with st.form("Параметры отчета \t\t"):
            ...
            revenue = st.number_input("Введите ваш доход от рекламы, если хотите посчитать ДРР")
            ...
            if submit_mult:
                if xx >= 0:
                    ...
                    if revenue > 0:
                        st.metric(label = "ДРР", value = str(round(((df_multiple_ctr_wrk_2.Cost.sum() * 100) / revenue), 2)) + "%")
    ...
    with tab_key_phrase:
        ...
        try:
            ...
            tab_key_mean, tab_key_median = st.tabs(["Анализ по среднему", "Анализ по медиане"])
